...
import { Activity, Bell, CalendarDays, Search, SlidersHorizontal, Sparkles } from "lucide-react";
...
export function Header({ title, subtitle, description }: { title: string; subtitle?: string; description?: string }) {
  ...
  return (
    <>
      <header className="mb-5 flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 font-mono text-[11px] font-bold text-amber-900 shadow-sm transition hover:bg-amber-100"
          >
            <Sparkles className="size-3.5 text-amber-600 animate-pulse" /> Judge Pitch & Demo Tour
          </button>
      ...
      <JudgeDemoModal isOpen={isDemoModalOpen} onClose={() => setIsDemoModalOpen(false)} />
    </>
  );
}
